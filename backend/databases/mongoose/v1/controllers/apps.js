...
//
const ctxArgs = {
  payload: { client: {}, conn: {}, model: { dynamic: true, name: "apps" } },
  params: { enable: { client: true } },
};
...
const update = async (req, res) => {
  try {
    ...
    const paramContext = { ...ctxArgs.params };
    const ctx = await getContext(payloadContext, paramContext);
    const { params, body, client, Model } = ctx;
    const { id } = params;

    const payloadOpt = { new: true, runValidators: true };
    const results = await Model.findByIdAndUpdate(id, body, payloadOpt);
    const format = [formatData(results)];
...
const findMany = async (req, res) => {
  try {
    ...
    const paramContext = { ...ctxArgs.params };
    const ctx = await getContext(payloadContext, paramContext);
    const { bulk: list, client, Model } = ctx;
  ...
  try {
    ...
    const methodSingle = async () => {
      for (const item of list) {
        try {
          const results = await Model.create(item);
    ...
    const methodSingle = async () => {
      for (const { id, ...item } of list) {
        try {
          ...
          const payloadOpt = { new: true, runValidators: true };
          const results = await Model.findByIdAndUpdate(id, item, payloadOpt);
