...
const methods = {
  ...
  update: async (payload = {}) => {
    const { Model, id, body, options } = payload;
    const opt = { new: true, runValidators: true, ...options };
    const results = await Model.findByIdAndUpdate(id, body, opt).toObject();
    return results;
  },
  ...
  createMany: async (payload = {}) => {
    const { Model, list, options } = payload;
    const opt = { ordered: false, ...options };
    const results = await Model.insertMany(list, opt);
    return results;
  },
  updateeMany: async (payload = {}) => {
    const { Model, query } = payload;
    const results = await Model.bulkWrite(query);
    return results;
  },
  deleteeMany: async (payload = {}) => {
    const { Model, query } = payload;
    const results = await Model.deleteMany(query);
    return results;
  },
};
...
const findMany = async (req, res) => {
  try {
    ...
    const results = Model.find(payloadQuery).lean();
