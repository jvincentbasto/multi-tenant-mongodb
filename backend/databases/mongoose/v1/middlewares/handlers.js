...
const getClient = async (payload = {}) => {
  ...
  const client = enable ? await dbHandler.getClient({ ...curPayload }) : null;
  return client;
};
const getDbConnection = async (payload = {}) => {
  const { req, res, enable = true, name } = payload;
  const { env, region, db } = req.params;

  const curPayload = {
    env: payload.env ?? env,
    region: payload.region ?? region,
    db: name ?? payload.db ?? db,
  };

  const conn = enable ? await dbHandler.connect({ ...curPayload }) : null;
  return conn;
};
const getModel = async (payload = {}) => {
  ...
  const Model = await fnHandler.getModel({ conn, name });
...
const getContext = async (payload = {}, params = {}) => {
  ...
  //
  const { db, model: name } = req.params;
  const curMDbName = payloadConn.name ?? db;
  ...
  let conn;
  if (enableConn) {
    conn = await getDbConnection({ req, name: curMDbName, ...payloadConn });
