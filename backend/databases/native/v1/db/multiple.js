...
// get client
const getMongoNativeClient = async (payload = {}) => {
...
const verifyDB = async (uri, name) => {
  ...
  try {
    const isAlive = isConnected(client);
    if (!isAlive) throw new Error("Invalid connection");
...
const connectMongoNative = async (payload = {}) => {
  try {
    ...
    const client = await getMongoNativeClient(uri);
...
export const multipleDb = {
  getClient: getMongoNativeClient,
  connect: connectMongoNative,
};
