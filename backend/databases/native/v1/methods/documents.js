...
const methods = {
  ...
  updateMany: async (...args) => {
    ...
    const methodBulk = async () => {
      ...
      try {
        ...
        failed.push({ error: error.message || "Bulk update failed" });
      }
    };
  ...
  deleteMany: async (...args) => {
    ...
    const methodSingle = async () => {
      for (const id of list) {
        try {
          ...
          rejects.failed.push({
            id,
            error: err?.message || "Document not deleted",
          });
        }
      }
    };
    ...
    const methodBulk = async () => {
      // 🔍 existing docs
      const existingDocs = await collection.find({ _id: { $in: list } });
      ...
      notFoundIds.forEach((id) => {
        rejects.failed.push({ id, error: "ID or document does not exist" });
      });
      ...
      if (existingIds.length > 0) {
        ...
        existingIds.forEach((id) => {
          if (remainingIds.has(id)) {
            rejects.failed.push({ id, error: "Document not deleted" });
          } else {
            data.push(id);
          }
        });
...
export const documents = generateMethods(...payloadMethods);
